...
export const sqlGenerationTool = createTool({
  ...
  execute: async ({ context: { naturalLanguageQuery, databaseSchema , llmId} }) => {
    try {
...
5. List of tables used`;
      ...
      // Get the LLM connection and create the AI model
      const llm = await LLMservice.getInstance();
      await llm.setdefaultllm(llmId);
      const llmdetails = await llm.getDefaultLLM();
    
      // Create the AI SDK model using the connection info
      const { createOpenAI } = await import('@ai-sdk/openai');
      const openai = createOpenAI({ apiKey: llmdetails!.key });
      const aiModel = openai(llmdetails!.model);
