...
export const databaseIntrospectionTool = createTool({
  ...
  execute: async ({context}) => {
    ...
    try {
      // Get the singleton instance and pass the connectionId as value , so the client gets initialized automatically
      ...
      const tables = await connser.executeQuery( tablesQuery);
      ...
      const columns = await connser.executeQuery(columnsQuery);
      ...
      const relationships = await connser.executeQuery(relationshipsQuery);
      ...
      const indexes = await connser.executeQuery(indexesQuery);

      // Get table row counts (sample)
      const rowCountsPromises = tables.map(async (table: any)  => {
        try {
          const countQuery = `SELECT COUNT(*) as row_count FROM "${table.schema_name}"."${table.table_name}";`;
          const result = await connser.executeQuery(countQuery);
          return {
